...
function verifyPKCE(key, challenge) {
...
router.get('/google/callback', oauthLimiter, passport_1.default.authenticate('google', { session: false, failureRedirect: '/login?error=oauth_failed' }), async (req, res) => {
    try {
        const user = req.user;
...
router.get('/github/callback', oauthLimiter, passport_1.default.authenticate('github', { session: false, failureRedirect: '/login?error=oauth_failed' }), async (req, res) => {
    try {
        const user = req.user;
        const token = jsonwebtoken_1.default.sign({ userId: user.id, email: user.email, role: user.role }, process.env.JWT_SECRET || 'fallback-secret', { expiresIn: '24h' });
        // Redirect to frontend with token
...
router.get('/microsoft/callback', oauthLimiter, passport_1.default.authenticate('microsoft', { session: false, failureRedirect: '/login?error=oauth_failed' }), async (req, res) => {
    try {
        const user = req.user;
        const token = jsonwebtoken_1.default.sign({ userId: user.id, email: user.email, role: user.role }, process.env.JWT_SECRET || 'fallback-secret', { expiresIn: '24h' });
        // Redirect to frontend with token
...
router.post('/link/:provider', oauthLimiter, async (req, res) => {
    try {
        ...
        const existingUser = await User_1.User.findByOAuthId(provider, oauthId);
        if (existingUser && existingUser.id !== userId) {
            return res.status(409).json({ error: 'OAuth account already linked to another user' });
        }
        // Link the account
        await User_1.User.linkOAuthAccount(userId, provider, oauthId);
...
router.post('/unlink/:provider', oauthLimiter, async (req, res) => {
    try {
        ...
        await User_1.User.updateOAuthInfo(userId, null, null);
